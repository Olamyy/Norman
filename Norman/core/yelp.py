...
from Norman.core.yelpAPI import Client3
from Norman.core.map import GoogleMap
...
# Yelp Auth API v2
from Norman.settings import YelpConfig
...
def yelp_search_v2(searchTerm, location, coordinates=None, limit=None, offset=0):
    ...
    if len(response.businesses):
        ...
        for biz in response.businesses:
            ...
            if coordinates is not None:
                business['distance'] = calculate_distance(coordinates, [biz.location.coordinate.latitude,
...
def yelp_search_v3(searchTerm, location, coordinates=None, limit=None, offset=0):
    ...
    if len(response['businesses']):
        ...
        for biz in response['businesses']:
            ...
            if coordinates is not None:
                business['distance'] = calculate_distance(coordinates, [biz['coordinates']['latitude'],
...
def google_place_search(searchTerm, location, coordinates=None):
    ...
    if len(results['businesses']):
        ...
        for biz in results:
            ...
            if coordinates is not None:
                business['distance'] = calculate_distance(coordinates, [biz['geometry']['location']['lat'],
